...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ message: 'subscribe_failed' }, { status: 400 });
    }

    return NextResponse.json({ ok: true, status: data?.status ?? 'ACTIVE' }, { status: 200 });
  } catch (err) {
